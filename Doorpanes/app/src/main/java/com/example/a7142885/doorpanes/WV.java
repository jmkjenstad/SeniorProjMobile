 ...
 *
 * This is the Week View class from the open source calendar framework used.  I modified this
 * class to add in a couple fields we needed.  The read me on alamkanak's GitHub page specifies
...
public class WV extends View {
    ...
    private WV.Direction mCurrentScrollDirection = WV.Direction.NONE;
    ...
    private List<WV.EventRect> mEventRects;
    ...
    private WV.Direction mCurrentFlingDirection = WV.Direction.NONE;
    ...
    // Listeners.
    private WV.EventClickListener mEventClickListener;
    private WV.EventLongPressListener mEventLongPressListener;
    private WV.MonthChangeListener mMonthChangeListener;
    private final GestureDetector.SimpleOnGestureListener mGestureListener = new GestureDetector.SimpleOnGestureListener() {
        ...
        public boolean onScroll(MotionEvent e1, MotionEvent e2, float distanceX, float distanceY) {
            if (mCurrentScrollDirection == WV.Direction.NONE) {
                if (Math.abs(distanceX) > Math.abs(distanceY)){
                    mCurrentScrollDirection = WV.Direction.HORIZONTAL;
                    mCurrentFlingDirection = WV.Direction.HORIZONTAL;
                }
                else {
                    mCurrentFlingDirection = WV.Direction.VERTICAL;
                    mCurrentScrollDirection = WV.Direction.VERTICAL;
                }
            }
        ...
        public boolean onFling(MotionEvent e1, MotionEvent e2, float velocityX, float velocityY) {
            ...
            if (mCurrentFlingDirection == WV.Direction.HORIZONTAL){
                mScroller.fling((int) mCurrentOrigin.x, 0, (int) velocityX, 0, Integer.MIN_VALUE, Integer.MAX_VALUE, 0, 0);
            }
            else if (mCurrentFlingDirection == WV.Direction.VERTICAL){
            ...
            ViewCompat.postInvalidateOnAnimation(WV.this);
            return true;
        }
        ...
        public boolean onSingleTapConfirmed(MotionEvent e) {
            if (mEventRects != null && mEventClickListener != null) {
                List<WV.EventRect> reversedEventRects = mEventRects;
                Collections.reverse(reversedEventRects);
                for (WV.EventRect event : reversedEventRects) {
    ...
    public WV(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        try {
            ...
            mEventPadding = a.getDimensionPixelSize(com.alamkanak.weekview.R.styleable.WeekView_hourSeparatorHeight, mEventPadding);
    ...
    private void drawTimeColumnAndAxes(Canvas canvas) {
        ...
        if (mCurrentScrollDirection == WV.Direction.VERTICAL) {
    ...
    private void drawHeaderRowAndEvents(Canvas canvas) {
        ...
        if (mCurrentScrollDirection == WV.Direction.HORIZONTAL) mCurrentOrigin.x -= mDistanceX;
        ...
        if (mEventRects != null) {
            for (WV.EventRect eventRect: mEventRects) {
    ...
    private class EventRect {
        public WVEvent event;
        public WVEvent originalEvent;
        public RectF rectF;
        public float left;
        public float width;
        public float top;
         ...
         */
        public EventRect(WVEvent event, WVEvent originalEvent, RectF rectF) {
            this.event = event;
            this.rectF = rectF;
    ...
    private void getMoreEvents(Calendar day) {
        ...
        if (mEventRects == null)
            mEventRects = new ArrayList<WV.EventRect>();
        ...
        if (mFetchedMonths[0] < 1 || mFetchedMonths[0] != previousMonth || mRefreshEvents) {
            if (!containsValue(lastFetchedMonth, previousMonth) && !isInEditMode()){
                List<WVEvent> events = mMonthChangeListener.onMonthChange((previousMonth==12)?day.get(Calendar.YEAR)-1:day.get(Calendar.YEAR), previousMonth);
                sortEvents(events);
                for (WVEvent event: events) {
                    cacheEvent(event);
                }
            }
        ...
        if (mFetchedMonths[1] < 1 || mFetchedMonths[1] != day.get(Calendar.MONTH)+1 || mRefreshEvents) {
            if (!containsValue(lastFetchedMonth, day.get(Calendar.MONTH)+1) && !isInEditMode()) {
                List<WVEvent> events = mMonthChangeListener.onMonthChange(day.get(Calendar.YEAR), day.get(Calendar.MONTH) + 1);
                sortEvents(events);
                for (WVEvent event : events) {
                    cacheEvent(event);
                }
        ...
        if (mFetchedMonths[2] < 1 || mFetchedMonths[2] != nextMonth || mRefreshEvents) {
            if (!containsValue(lastFetchedMonth, nextMonth) && !isInEditMode()) {
                List<WVEvent> events = mMonthChangeListener.onMonthChange(nextMonth == 1 ? day.get(Calendar.YEAR) + 1 : day.get(Calendar.YEAR), nextMonth);
                sortEvents(events);
                for (WVEvent event : events) {
                    cacheEvent(event);
                }
        ...
        ArrayList<WV.EventRect> tempEvents = new ArrayList<WV.EventRect>(mEventRects);
        mEventRects = new ArrayList<WV.EventRect>();
        ...
        while (dayCounter.getTimeInMillis() <= maxDay.getTimeInMillis()) {
            ArrayList<WV.EventRect> eventRects = new ArrayList<WV.EventRect>();
            for (WV.EventRect eventRect : tempEvents) {
    ...
    private void cacheEvent(WVEvent event) {
        if (!isSameDay(event.getStartTime(), event.getEndTime())) {
            ...
            WVEvent event1 = new WVEvent(event.getId(), event.getName(), event.getStartTime(), endTime, event.getDescription(), event.getOwner());
            event1.setColor(event.getColor());
            WVEvent event2 = new WVEvent(event.getId(), event.getName(), startTime, event.getEndTime(), event.getDescription(), event.getOwner());
            event2.setColor(event.getColor());
            mEventRects.add(new WV.EventRect(event1, event, null));
            mEventRects.add(new WV.EventRect(event2, event, null));
        }
        else
            mEventRects.add(new WV.EventRect(event, event, null));
    }
     ...
     */
    private void sortEvents(List<WVEvent> events) {
        Collections.sort(events, new Comparator<WVEvent>() {
            @Override
            public int compare(WVEvent event1, WVEvent event2) {
     ...
     */
    private void computePositionOfEvents(List<WV.EventRect> eventRects) {
        // Make "collision groups" for all events that collide with others.
        List<List<WV.EventRect>> collisionGroups = new ArrayList<List<WV.EventRect>>();
        for (WV.EventRect eventRect : eventRects) {
            boolean isPlaced = false;
            outerLoop:
            for (List<WV.EventRect> collisionGroup : collisionGroups) {
                for (WV.EventRect groupEvent : collisionGroup) {
            ...
            if (!isPlaced) {
                List<WV.EventRect> newGroup = new ArrayList<WV.EventRect>();
     ...
     */
    private void expandEventsToMaxWidth(List<WV.EventRect> collisionGroup) {
        // Expand the events to maximum possible width.
        List<List<WV.EventRect>> columns = new ArrayList<List<WV.EventRect>>();
        columns.add(new ArrayList<WV.EventRect>());
        for (WV.EventRect eventRect : collisionGroup) {
            boolean isPlaced = false;
            for (List<WV.EventRect> column : columns) {
                if (column.size() == 0) {
            ...
            if (!isPlaced) {
                List<WV.EventRect> newColumn = new ArrayList<WV.EventRect>();
        ...
        for (int i = 0; i < maxRowCount; i++) {
            ...
            for (List<WV.EventRect> column : columns) {
                if (column.size() >= i+1) {
                    WV.EventRect eventRect = column.get(i);
     ...
     */
    private boolean isEventsCollide(WVEvent event1, WVEvent event2) {
        long start1 = event1.getStartTime().getTimeInMillis();
        long end1 = event1.getEndTime().getTimeInMillis();
        long start2 = event2.getStartTime().getTimeInMillis();
        long end2 = event2.getEndTime().getTimeInMillis();
    ...
    private void deleteFarMonths(Calendar currentDay) {
        ...
        List<WV.EventRect> newEvents = new ArrayList<WV.EventRect>();
        for (WV.EventRect eventRect : mEventRects) {
    ...
    public void setOnEventClickListener (WV.EventClickListener listener) {
        this.mEventClickListener = listener;
    }

    public WV.EventClickListener getEventClickListener() {
        return mEventClickListener;
    }

    public WV.MonthChangeListener getMonthChangeListener() {
        return mMonthChangeListener;
    }

    public void setMonthChangeListener(WV.MonthChangeListener monthChangeListener) {
        this.mMonthChangeListener = monthChangeListener;
    }

    public WV.EventLongPressListener getEventLongPressListener() {
        return mEventLongPressListener;
    }

    public void setEventLongPressListener(WV.EventLongPressListener eventLongPressListener) {
    ...
    public boolean onTouchEvent(MotionEvent event) {
        if (event.getAction() == MotionEvent.ACTION_UP) {
            ...
            if (mCurrentScrollDirection == WV.Direction.HORIZONTAL) {
            ...
            mCurrentScrollDirection = WV.Direction.NONE;
        }
    ...
    public void computeScroll() {
        ...
        if (mScroller.computeScrollOffset()) {
            if (Math.abs(mScroller.getFinalX() - mScroller.getCurrX()) < mWidthPerDay + mColumnGap && Math.abs(mScroller.getFinalX() - mScroller.getStartX()) != 0) {
                ...
                if (mCurrentFlingDirection == WV.Direction.VERTICAL) mCurrentOrigin.y = mScroller.getCurrY();
    ...
    public interface MonthChangeListener {
        public List<WVEvent> onMonthChange(int newYear, int newMonth);
    }
